# Quorum — Semantic Content Moderation on GenLayer

Quorum is a public community board with **no central moderator**. Anyone can
...
rather than byte-equality**. Quorum's state transitions intentionally depend
on:
...
trusted moderator, and it is the only part Quorum puts on GenLayer.
...
```
contract/quorum.py               # the Intelligent Contract
...
- **Agree on a coarse, categorical signal**, never free-form LLM text, or
  consensus stalls. Quorum agrees on `APPROVE`/`REJECT` and carries the prose
...
Quorum today ships the part that can only exist on GenLayer — semantic
